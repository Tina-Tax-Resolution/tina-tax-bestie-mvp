...
function profitAlertHtml(path) {
  ...
  if (path.lossRows.length === 2) {
    return `<div class="alert warn"><strong>Bestie Alert:</strong> Your prior-year records show 2 loss years in this five-year lookback. This is an early warning, not a tax determination. That can happen while growing, but keep receipts, proof, income efforts, and notes showing what you changed to improve profit.${trendText}<ul class="profit-path-list">${yearSummary}</ul>The 9-question Business Check-In opens when saved records show losses in 3 of 5 prior tax years. Educational record organization only; consult a qualified tax professional.</div>`;
  }
